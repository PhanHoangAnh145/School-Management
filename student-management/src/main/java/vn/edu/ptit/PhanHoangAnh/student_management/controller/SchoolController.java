...
public class SchoolController {
    ...
    @GetMapping()
    public ResponseEntity<ApiResponse<List<SchoolResponseDTO>>> findAllSchool() {
        List<SchoolResponseDTO> schoolList =  this.schoolService.findAllSchool();
        return ApiResponse.success(schoolList);

    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<SchoolResponseDTO>> findSchoolById(@PathVariable Long id) {
        SchoolResponseDTO school = this.schoolService.findSchoolById(id);
        return ApiResponse.success(school);
    }

    @PostMapping()
    public ResponseEntity<ApiResponse<SchoolResponseDTO>> saveSchool(@RequestBody School school) {
        SchoolResponseDTO schoolSave = this.schoolService.saveSchool(school);
    ...
    public ResponseEntity<ApiResponse<String>> updateSchoolById(@PathVariable Long id, @RequestBody School school) {
        SchoolResponseDTO schoolUpdate = this.schoolService.updateSchoolById(id, school);
